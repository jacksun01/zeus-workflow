...
class GetOtherTokenSerializer(serializers.Serializer):

    auth_code = serializers.CharField(required=True,label='认证码', help_text='认证码',allow_blank=False,allow_null=False)
    email = serializers.CharField(required=True,label='账户邮箱', help_text='账户邮箱',allow_blank=False,allow_null=False)

    def validate(self,attrs):
        if attrs["auth_code"] != settings.AUTH_CODE:
            raise ValidationError('认证码错误')
        user = User.objects.filter(email=attrs["email"])
        if not user.exists():
            raise ValidationError('此邮箱账户未登陆过')
        return attrs